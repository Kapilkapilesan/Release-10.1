...
export function CollectionStats({ stats, isLoading }: CollectionStatsProps) {
    if (isLoading) {
        return (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                    <div key={i} className="bg-card rounded-2xl p-4 border border-border-default animate-pulse">
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-2">
            {/* Total Expectation */}
            <div className="bg-card rounded-2xl p-4 border border-border-default/50 transition-all group">
                <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform" style={{ backgroundColor: `${colors.primary[500]}1a`, color: colors.primary[600] }}>
                ...
                <p className="text-2xl font-black text-text-primary tracking-tight">LKR {stats.totalTarget.toLocaleString()}</p>
            </div>

            {/* Actual Collection */}
            <div className="bg-card rounded-2xl p-4 shadow-sm border border-border-default/50 transition-all hover:shadow-xl group">
                <div className="flex items-center gap-3 mb-2">
                ...
                <p className="text-2xl font-black text-emerald-500 tracking-tight">LKR {stats.totalCollected.toLocaleString()}</p>
            ...
            <div className="bg-card rounded-2xl p-4 shadow-sm border border-border-default/50 transition-all hover:shadow-xl group">
                <div className="flex items-center gap-3 mb-2">
                    <div className={`w-10 h-10 ${isPositiveVariance ? 'bg-emerald-500/10 text-emerald-500' : 'bg-rose-500/10 text-rose-500'} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform`}>
                        {isPositiveVariance ? (
                            <TrendingUp className="w-5 h-5" />
                        ) : (
                ...
                <p className={`text-2xl font-black tracking-tight ${isPositiveVariance ? 'text-emerald-500' : 'text-rose-500'}`}>
            ...
            <div className="bg-card rounded-2xl p-4 shadow-sm border border-border-default/50 transition-all hover:shadow-xl group">
                ...
                <p className="text-2xl font-black text-text-primary tracking-tight">{stats.totalDueCustomers.toLocaleString()}</p>
            </div>

            {/* Paid Customers */}
            <div className="bg-card rounded-2xl p-4 shadow-sm border border-border-default/50 transition-all hover:shadow-xl group">
                <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 bg-indigo-500/10 text-indigo-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                ...
                <p className="text-2xl font-black text-text-primary tracking-tight">{stats.totalPaidCustomers.toLocaleString()}</p>
            </div>

            {/* Active Portfolio */}
            <div className="bg-card rounded-2xl p-4 shadow-sm border border-border-default/50 transition-all hover:shadow-xl group">
                <div className="flex items-center gap-3 mb-2">
                    <div className="w-10 h-10 bg-rose-500/10 text-rose-500 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                ...
                <p className="text-2xl font-black text-text-primary tracking-tight">{stats.totalActiveCustomers.toLocaleString()}</p>
            </div>
        </div>
    );
}
